...
exports.getCopyPlugin = function (isdev) {
  ...
  const { paths: { src }, env, [env]: { assetsRoot, publicPath } } = require('./config')
  const manifsetImg = {}
  ...
  return new CopyWebpackPlugin([{
    ...
    transform (content, filepath) {
      ...
      manifsetImg[publicPath + imgPathNoHash] = publicPath + imgPath
...
exports.getManifestPlugin = function (path, publicPath, ext = 'js') {
    ...
    */
    generate: function (manifest, files) {
      files.forEach(file => {
        ...
        const name = $path.join(publicPath, file.name.replace(path, ''))

        manifest[name] = url.replace(path, publicPath)
      })
      return manifest
    }
  })
